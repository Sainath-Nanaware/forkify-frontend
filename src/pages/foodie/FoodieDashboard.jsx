...
function FoodieDashboard() {
  ...
  const drawerItem=[
    { text:'Profile', route:'/foodie/profile'},
    { text: 'Saved', route:'/foodie/savedRecipes' },
  ...
  return (
    ...
    <div className=' h-[122vh] pt-6 flex justify-center items-start flex-wrap gap-6'>
        ...
        {allRecipesArray && allRecipesArray.map((element)=>{
            return <Link to={`/foodie/recipeDetails/${element._id}`} key={element._id}> <RecipeCard  name={element.title} desc={element.description} img={element.image} /></Link>
        })}
